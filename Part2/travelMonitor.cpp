...
{
    int numMonitors;
    unsigned long bufferSize,sizeOfBloom;
    string input_dir;
    int pos;
    ...
    if ((pos=findValidFlag(argv,"-b"))>0)//Find position of the "-b" flag in the arguments
    {
        if (isNum(argv[pos+1])) //Check that next argument is a number
        {
            bufferSize=(unsigned long) (atol(argv[pos+1]));
            if (bufferSize<=0)
            {
                cout<<"bufferSize is not a positive number."<<endl;
                return -3;
            }
        }
        else
    ...
    AppTravelMonitor App(numMonitors,bufferSize,sizeOfBloom,input_dir);//Create and initialize an

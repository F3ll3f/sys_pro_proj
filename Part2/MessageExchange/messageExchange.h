...
/*Messages are exchanged using a given buffer and following this protocol: A message is
 ...
 the fifo one-by-one. Using the given buffer the receiver receives the 10 first bytes,
...
bool GetMessage(int NamedPipe,char *buf,unsigned long bufferSize,
                char *&Message,unsigned long &mesSize);/*Using the given buffer, get a message
from a named pipe which is opened in the "NamedPipe" file descriptor. Return false if failed.*/
                                                                              

bool SendMessage(int NamedPipe,char *buf,unsigned long bufferSize,
                 const char *Message,unsigned long mesSize);/*Using the given buffer, send a
message to a named pipe which is opened in the "NamedPipe" file descriptor. Return false if
failed.*/

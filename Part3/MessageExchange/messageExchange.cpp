...
bool GetMessage(int Socket,char *buf,unsigned long bufferSize,char *&Message,unsigned long &mesSize)
{
    ...
    while (index<10) //Read first 10 bytes that store the size of Message. Read until all 10 bytes have been received.
    {
        ...
        bytes_read=read(Socket,buf,min_c_buf);
...
bool SendMessage(int Socket,char *buf,unsigned long bufferSize,const char *Message,unsigned long mesSize)
{
    ...
    while (bytes_copied<(mesSize+10)) //Continue writing in the socket until the whole message has been sent
    {   //Message will be splitted into parts. Each part has bufferSize bytes(or less for the last part)
        ...
        {                                   //been copied to buffer has been sent (or the whole message has been sent)
            ...
            bytes_written=write(Socket,buf,min_c_buf); // bytes(rest of message)

...
private:
    int *Sockets; //Sockets[i] is a socket that will be used to exchange messages
                  //with monitor i
...
public:
    AppTravelMonitor(int numMonitors,int numThreads,unsigned long sockBufferSize,unsigned long cyclicBufferSize,
                     unsigned long sizeOfBloom,std::string input_dir);
    ~AppTravelMonitor();
...
private:
    void CreateMonitorReplacingChild(int Monitor,int port,int numThreads,unsigned long cyclBufferSize,
                unsigned long sizeOfBloom); /*Replace current child process with a monitorServer process*/
    void ConnectWithMonitor(int Monitor,int port); /* Create a TCP connection with the Monitor*/
    char *ReceiveMessageFromMonitor(unsigned long &mesSize,int Monitor);/* Receives a message from
    "Monitor" from the respective socket using the buffer. The merged(original) message is returned
    ...
    "Monitor" through the respective socket using the buffer according to the protocol in the
    ...
    int FindMaxSocket();//Return the maximum integer of the socket descriptors.
